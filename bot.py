...
from telegram.ext import (
    ...
    MessageHandler,
    ContextTypes,
    filters,  # ✅ v20+ uses lowercase 'filters'
)
...
messages = {
    ...
    'Kaspi': (
        "Для оплаты с помощью Kaspi, отправьте платеж на номер: +77085000115.\n"
        "Для оплаты переводом (KZ), отправьте платеж на номер карты: 4400430207827236\n"
        "Как только вы это сделаете, отправьте скриншот в этот чат.\n"
        "При оплате, пожалуйста, укажите в сообщении ваш тег в Telegram."
    ),
    'Zelle': (
    ...
    'Перевод': (
        "Для оплаты переводом, отправьте платеж на номер карты:\n"
...
async def career_lab(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Готовы присоединиться? Выберите /start. Стоимость: $20"
    )
...
async def career_lab_pro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Готовы присоединиться? Выберите /start. Стоимость: $60"
    )

async def command3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Career Lab — это профессиональное коммьюнити для студентов и молодых специалистов.\n"
        "Мы — Лиза, Кирилл и Даша, собрали самые полезные ресурсы и объединили наш опыт и знания, чтобы помочь вам приобрести необходимые навыки.\n"
        "Чтобы присоединиться, Вам необходимо выбрать тариф (базовый или pro), и произвести оплату.\n"
        "Выберите /start"
    )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.PHOTO, photo_handler))  # ✅ fixed filters usage
